...
void commandCallback(String &msg)
{
    if (msg.indexOf(cmd_foodtreat_dispense) == 1)
    {
        ...
        if (tray_error)
        {
            mgschwan_sendStringTCP(String::format("@error;"));
        }
        else {
            if (foodtreat_taken) mgschwan_sendStringTCP(String::format("@ok:taken:;"));
            else mgschwan_sendStringTCP(String::format("@ok:not_taken:;"));
        }
    }
    if (msg.indexOf(cmd_check_buttons) == 1)
    {
        mgschwan_sendStringTCP(String::format("@buttons:%d:;",hubButtonPressed));
        hubButtonPressed = 0;
    }
    if (msg.indexOf(cmd_play_audio) == 1)
    {
        ...
        mgschwan_sendStringTCP(String("@ok;"));
    }
...
void loop()
{
    ...
    if (system_ready) 
    {
        ...
        if (buttons != 0)
        {
            ...
            {
                mgschwan_sendStringTCP(String::format("@button_event:%d:;",buttons));
        ...
        {
            commandCallback(message);
            Log.info(String("TCP Received: ")+message);
        }
    }
}
